...
- **Filterable comparison matrix** (`/`) — all cards as columns, key attributes as
  rows (fees, FX, category earn rates, lounge access, perks). Filter by network,
  reward type, no-FX-fee, and lounge access; the best value in each numeric row is
  highlighted green and the worst red — recomputed over the filtered set, with ties
...
**Filterable comparison matrix** (`/`) — every card side-by-side with a filter bar
(network, reward type, no-FX-fee, lounge access) and best/worst cells highlighted
green/red:

![Comparison matrix showing eight Canadian credit cards side-by-side with a filter bar and best/worst cells highlighted](docs/screenshots/home.png)
...
├── components/  # Client UI island (ComparisonMatrix — filters + table)
...
*separately* (a `redemption` flexibility score, shown as its own row and used as a
